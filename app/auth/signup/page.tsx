...
export default function SignupPage() {
  ...
  const [age, setAge] = useState("")        // ✅ AJOUT AGE
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!age || ageNumber < 10 || ageNumber > 120) {
    ...
    try {
      const response = await fetch("/api/auth/signup", {
        ...
        body: JSON.stringify({ email, password, username, age }),  // ✅ ENVOI AGE
      })
      ...
      <div className="w-full max-w-md px-4">
        <div className="card-premium space-y-6">
          ...
          <div className="text-center text-sm text-muted-foreground">
            ...
            <Link href="/auth/login" className="text-accent font-semibold hover:text-accent/80 transition underline">
              Se connecter
            </Link>
          </div>
        </div>
